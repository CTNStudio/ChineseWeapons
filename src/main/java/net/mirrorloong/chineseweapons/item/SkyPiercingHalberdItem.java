...
public class SkyPiercingHalberdItem extends FourBlockReachSwordItem {
    ...
    public boolean hurtEnemy(ItemStack stack, LivingEntity target, LivingEntity attacker) {
        ...
        if (player.isShiftKeyDown()) {
            ...
            Holder<MobEffect> bleedEffect = ChineseWeaponsModEffects.BleedEffectSupplier;
            if (!target.hasEffect(bleedEffect)) {
                target.addEffect(new MobEffectInstance(bleedEffect, 5 * 20, 0, false, true));
            }

            if (player.getServer() != null) {
                var advancementLookup = player.getServer().getAdvancements();
